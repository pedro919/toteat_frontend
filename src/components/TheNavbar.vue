...
const navigation = ref([
  ...
  { name: 'Estádisticas', to: '/estadisticas', current: false },
...
const changeNavigationItemHandler = (name) => {
...
<template>
  <div>
    <Disclosure v-slot="{ open }" as="nav" class="bg-gray-800">
      <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 md:flex md:flex-row md:justify-center">
              ...
              <div class="flex space-x-4">
                <div v-for="item in navigation" :key="item.name">
                  <router-link :to="item.to">
                    <button class="px-3 py-2 rounded-md text-sm font-medium" :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white']" @click="changeNavigationItemHandler(item.name)">
                      {{ item.name }}
                    </button>
                  </router-link>
                </div>
